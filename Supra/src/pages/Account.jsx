...
const AccountDashboard = () => {
  ...
  const Navigation = () => (
    <nav className="space-y-2">
      <button 
        className={`w-full text-left py-2 px-4 ${currentView === 'dashboard' ? 'bg-gradient-to-r from-orange-500 to-pink-500 text-white' : 'hover:bg-gray-100'}`}
        onClick={() => setCurrentView('dashboard')}
      >
        Dashboard
      </button>
      <button 
        className={`w-full text-left py-2 px-4 ${currentView === 'addresses' ? 'bg-gradient-to-r from-orange-500 to-pink-500 text-white' : 'hover:bg-gray-100'}`}
